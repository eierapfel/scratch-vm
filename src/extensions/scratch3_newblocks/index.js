...
class Scratch3NewBlocks {
  constructor(runtime) {
    ...
    if (this._viewer.canDrawCanvas(this._viewer._image.src)) {
      console.log("do drawcanvas");
      this._viewer.drawCanvas();
    }
    else {
      console.log("do not draw canvas");
    }
  ...
  getInfo() {
    return {
      ...
      blocks: [
        ...
        {
          opcode: 'selectImage',
          text: formatMessage({ id: 'newblocks.selectImage', default: ' [IP] の映像を描画する' }),
          blockType: BlockType.COMMAND,
          arguments: {
            IP: { type: ArgumentType.STRING, defaultValue: formatMessage({ id: 'newblocks.selectImage', default: 'localhost' }) }
          },
        },
      ],
      menus: {
  ...
  selectImage(args) {
    ...
    this._viewer.selectImageURL(ip);
  }
